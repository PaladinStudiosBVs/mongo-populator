...
class CLI(object):
    ...
    def parse(self):
        ...
        destination_group.add_argument('--destination-ssh-host', dest='destination_ssh_host', default=C.DESTINATION_SSH_HOST,
                                  action='store',
                                  help='SSH host we\'re connecting to if we decide to use SSH for the source')
        destination_group.add_argument('--destination-ssh-user', dest='destination_ssh_user', default=C.DESTINATION_SSH_USER,
                                  action='store', help='SSH user to connect to destination')
    ...
    def run(self):
        ...
        if self.options['source_use_local_dump']:
            ...
            source = SSHSource(
                db_name=self.options['source_db_name'],
                db_user=self.options['source_db_user'],
                db_password=self.options['source_db_password'],
                ssh_host=self.options['source_ssh_host'],
                ssh_user=self.options['source_ssh_user'],
                ssh_password=self.options['source_ssh_password'],
                key_file=self.options['source_ssh_key_file'],
                tmp_dir=self.options['source_tmp_dir'],
            )
        ...
        if self.options['destination_use_local_db']:
            ...
            destination = SSHDestination(
                ...
                key_file=self.options['destination_ssh_key_file'],
                source=source,
            )
        else:
